...
import { agents, agentVersions, teamMembers, scenarioExecutions } from '@/lib/db/schema';
...
export async function POST(
  ...
  try {
    ...
    // Create a temporary scenario execution for testing
    // (Agents normally run within scenarios, but for testing we create a test execution)
    const [testScenarioExecution] = await db.insert(scenarioExecutions).values({
      scenarioVersionId: activeVersion.id, // Reuse version ID for simplicity
      status: 'running',
      input,
